...
def getTradeHistory(exchange, currencypair):
    ...
    if exchange == POLONIEX:
        ...
        for d in data:
            for b in d['data']:
                history.append([b['price'], datetime.datetime.fromtimestamp((float(b['ts']) / 1000) + time.timezone )])
        ...
        history = [[int(d['price']), datetime.datetime.fromtimestamp(float(d['date']) + time.timezone)] for d in data]
    elif exchange == OKEX:
        ...
        pair = currencypair.split('_')
        data = json.loads(requests.get(
            'https://bittrex.com/api/v1.1/public/getmarkethistory?market={}'.format('{}-{}'.format(pair[1], pair[0]))).text)
        ...
        history = [ [d['price'] , datetime.datetime.fromtimestamp( float(( d['time'])/1000) + time.timezone )] for d in data ]
...
#plot(getTradeHistory(HUOBI, 'ETH_USDT'),getTradeHistory(POLONIEX,'ETH_USDT'))
